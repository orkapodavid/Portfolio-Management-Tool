...
def goal_summary_row(goal: Goal) -> rx.Component:
    return rx.el.div(
        rx.el.div(
            rx.el.div(
                rx.icon(goal["icon"], size=16, class_name=f"text-{goal['color']}-600"),
                class_name=f"p-1.5 rounded-lg bg-{goal['color']}-100 mr-3",
            ),
            rx.el.div(
                rx.el.p(
                    goal["name"], class_name="text-sm font-bold text-gray-900 truncate"
                ),
                rx.el.p(
                    f"${goal['current_amount'] / 1000:.0f}k / ${goal['target_amount'] / 1000:.0f}k",
        ...
        rx.el.div(
            rx.el.div(
                class_name=f"h-full bg-{goal['color']}-500",
                style={
                    "width": f"{goal['current_amount'] / goal['target_amount'] * 100}%"
                },
            ),
            class_name=f"w-20 h-1.5 rounded-full bg-gray-100 overflow-hidden",
        ),
        class_name="flex items-center justify-between py-3 border-b border-gray-50 last:border-0",
    )


def goals_summary_widget() -> rx.Component:
    return rx.el.div(
        rx.el.div(
            rx.el.h3("Financial Goals", class_name="text-lg font-bold text-gray-900"),
            rx.el.a(
                "View All",
                href="/goals",
                class_name="text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 px-2.5 py-1 rounded-lg transition-colors",
            ),
            class_name="flex items-center justify-between mb-4",
        ),
        rx.el.div(
            rx.foreach(GoalsState.top_goals, goal_summary_row),
        ...
        rx.el.div(
            rx.el.div(
                rx.el.span(
                    "Total Saved",
                    class_name="text-xs text-gray-500 font-bold uppercase tracking-wide",
                ),
                rx.el.span(
            ...
            rx.el.div(
                ...
                rx.el.span(
                    f"{GoalsState.goals_on_track} Goals",
                    class_name="text-lg font-bold text-emerald-600",
                ),
                class_name="flex flex-col text-right",
            ),
            class_name="flex items-center justify-between pt-4 border-t border-gray-100",
        ),
        class_name="bg-white rounded-3xl p-6 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] h-fit",
    )