"""
Risk Input Recon Mixin - Force Refresh Pattern

Uses force refresh button instead of auto-refresh for static data.
"""

import reflex as rx
from app.services import DatabaseService
from app.states.reconciliation.types import RiskInputReconItem


class RiskInputReconMixin(rx.State, mixin=True):
    """
    Mixin providing Risk Input Recon data state with force refresh pattern.
    """

    risk_input_recon: list[RiskInputReconItem] = []
    is_loading_risk_input_recon: bool = False
    risk_input_recon_error: str = ""

    # Status bar state (per-tab)
    risk_input_recon_last_updated: str = "—"

    risk_input_recon_search: str = ""

    async def load_risk_input_recon_data(self):
        """Load Risk Input Recon data."""
        self.is_loading_risk_input_recon = True
        self.risk_input_recon_error = ""
        try:
            service = DatabaseService()
            self.risk_input_recon = await service.get_risk_input_recon()
        except Exception as e:
            self.risk_input_recon_error = str(e)
            import logging

            logging.exception(f"Error loading Risk Input Recon: {e}")
        finally:
            self.is_loading_risk_input_recon = False
            from datetime import datetime

            self.risk_input_recon_last_updated = datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S"
            )

    async def force_refresh_risk_input_recon(self):
        """Force refresh - reloads data from service (all cells flash).
        
        Uses yield + is_loading guard to:
        1. Prevent multiple clicks while loading
        2. Show loading overlay immediately
        """
        if self.is_loading_risk_input_recon:
            return  # Debounce: ignore clicks while loading
        
        import asyncio
        self.is_loading_risk_input_recon = True
        yield  # Send loading state to client immediately
        await asyncio.sleep(0.5)  # Brief delay to show loading overlay
        await self.load_risk_input_recon_data()

    def set_risk_input_recon_search(self, query: str):
        self.risk_input_recon_search = query

    @rx.var(cache=True)
    def filtered_risk_input_recon(self) -> list[RiskInputReconItem]:
        data = self.risk_input_recon
        if self.risk_input_recon_search:
            query = self.risk_input_recon_search.lower()
            data = [
                item
                for item in data
                if query in item.get("ticker", "").lower()
                or query in item.get("underlying", "").lower()
            ]
        return data
